...
- **Dynamic SEO meta**: `dynamicTitle` / `dynamicDescription` computeds embed the origin, destination, and — when a precomputed route is available via `getCachedRoute()` — a concrete fare into the `<title>` and `<meta description>`. This is why `plugins/seed-cache.client.ts` is an async plugin: it must populate the cache before the description first evaluates. The fare comes from `estimateTripFare()` (route distance in km, its tunnels with the return toll, the suggested taxi type), and a shared-link load (`apply-suggested-taxi-type`) starts the calculator on that suggested type once, so the two agree. Later trips only show the suggestion banner.
...
- `@nuxt/scripts`: Third-party script loaders — provides `useScriptGoogleAnalytics` (GA4) and `useScriptPostHog`, both deferred via `trigger: 'onNuxtReady'`. In development the trigger becomes `'manual'` so the live ids are never loaded: there is no separate dev property, and every `npm run dev` or browser-driver visit would otherwise land in the real dashboards as a new first-time user. The registry entries stay in place, so `useAnalytics()` still resolves its proxies.
