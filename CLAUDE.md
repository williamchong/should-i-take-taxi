...
- **Dynamic SEO meta**: `dynamicTitle` / `dynamicDescription` computeds embed the origin, destination, and — when a precomputed route is available via `getCachedRoute()` — a concrete fare into the `<title>` and `<meta description>`. The fare comes from `estimateTripFare()` (route distance in km, its tunnels with the return toll, the suggested taxi type), and a shared-link load (`apply-suggested-taxi-type`) starts the calculator on that suggested type once, so the two agree. Later trips only show the suggestion banner. This is why `plugins/seed-cache.client.ts` is an async plugin: it must populate the cache before the description first evaluates.
